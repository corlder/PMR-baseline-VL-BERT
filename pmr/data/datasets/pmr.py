...
class PMRDataset(Dataset):
    def __init__(self, ann_file, image_set, root_path, data_path, transform=None, task='Q2A', test_mode=False,
                 ...
                 **kwargs):
        """
         Premise-based Multimodal Reasoning Dataset

        :param ann_file: annotation jsonl file
        :param image_set: image folder name, e.g., 'pmr1images'
        :param root_path: root path to cache database loaded from annotation file
        :param data_path: path to pmr dataset
        :param transform: transform
        :param task: 'Q2A' means question to answer
        ...
        :param kwargs:
        """
        super(PMRDataset, self).__init__()
    ...
    def load_annotations(self, ann_file):
        tic = time.time()
        database = []
        db_cache_name = 'pmr_nometa_{}_{}_{}'.format(self.task, self.image_set, os.path.basename(ann_file)[:-len('.jsonl')])
...
#                 }
                db_i = {
                    'annot_id': ann['total_id'],
                    # 'annot_id': ann['annot_id'],
                    'objects': ann['objects'],
                    'img_fn': img_fn,
                    'metadata_fn': metadata_fn,
#                     'question': ann['question'],
                    'question':ann['premise'],
                    'answer_choices': ann['answer_choices'],
                    'answer_label': ann['answer_label'] if not self.test_mode else None,
#                     'rationale_choices': ann['rationale_choices'],
#                     'rationale_label': ann['rationale_label'] if not self.test_mode else None,
