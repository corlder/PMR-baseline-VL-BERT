...
from pmr.data.build import make_dataloader
from pmr.function.config import config, update_config
from pmr.modules import *
from pmr.function.val import joint_validation
...
def parse_args():
    ...
    parser.add_argument('--result-path', type=str, default='./pmr_val_results')
    parser.add_argument('--result-name', type=str, default='vl-bert')
    parser.add_argument('--use-cache', default=False, action='store_true')
    parser.add_argument('--annot', type=str, default='./data/pmr/val.jsonl')
...
def main():
    ...
    with jsonlines.open(args.annot) as reader:
        gts = [(obj['answer_label'], obj.get('rationale_label',0)) for obj in reader]
    a_gt = np.array([gt[0] for gt in gts], dtype=np.int64)
    r_gt = np.array([gt[1] for gt in gts], dtype=np.int64)

    # cache
    result_csv = os.path.join(args.result_path, '{}.csv'.format(args.result_name))
        ...
        if a_config is not None and args.a_ckpt is not None:
            ...
            # for a_batch in answer_loader:
                ...
                if a_batch_pred.dim() == 2:
                    a_pred[i_sample:(i_sample + batch_size)] = a_batch_pred.detach().cpu().numpy().astype(np.float,copy=False)
            ...
            a_pred = sigmoid(a_pred)
            _ids = list(range(a_pred.shape[0]))
            _proba = [item.tolist() for item in a_pred]
            _label = [np.argmax(item) for item in a_pred]
            df = pd.DataFrame({'ids':_ids, 'proba':_proba, 'label':_label})
    ...
    if a_pred is not None:
        print("Dataset size is:{}".format(a_gt.size))
